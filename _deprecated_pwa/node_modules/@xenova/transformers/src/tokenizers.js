 ...
 * should implement the `p<RESEND_API_KEY>` method to define the specific pre-tokenization logic.
 * @extends Callable
 */
class PreTokenizer extends Callable {
   ...
   */
     ...
     */
    p<RESEND_API_KEY>(text, options) {
        throw Error("p<RESEND_API_KEY> should be implemented in subclass.")
    }
    ...
    p<RESEND_API_KEY>(text, options) {
        return (Array.isArray(text)
            ? text.map(x => this.p<RESEND_API_KEY>(x, options))
            : this.p<RESEND_API_KEY>(text, options)
        ).flat();
    }

    /**
     * Alias for {@link PreTokenizer#p<RESEND_API_KEY>}.
     * @param {string|string[]} text The text or array of texts to pre-tokenize.
     * @param {Object} [options] Additional options for the pre-tokenization logic.
     * @returns {string[]} An array of pre-tokens.
     */
    _call(text, options) {
        return this.p<RESEND_API_KEY>(text, options);
    }
}
...
class BertPreTokenizer extends PreTokenizer {
    ...
    constructor(config) {
        ...
        // https://github.com/huggingface/tokenizers/blob/b4fcc9ce6e4ad5806e82826f816acfdfdc4fcc67/tokenizers/src/p<RESEND_API_KEY>/bert.rs#L11
...
class PreTokenizerSequence extends PreTokenizer {
    ...
    p<RESEND_API_KEY>(text, options) {
        ...
        return this.tokenizers.reduce((preTokenizedText, tokenizer) => {
            return tokenizer.p<RESEND_API_KEY>(preTokenizedText, options);
        }, [text]);
    }
}
...
export class PreTrainedTokenizer extends Callable {
    ...
    constructor(tokenizerJSON, tokenizerConfig) {
        ...
        this.p<RESEND_API_KEY> = PreTokenizer.fromConfig(tokenizerJSON.p<RESEND_API_KEY>);
    ...
    _encode_text(text) {
        ...
        const tokens = sections.map((x, section_index) => {
            ...
            if (addedToken !== undefined) {
                ...
                const sectionTokens = (this.p<RESEND_API_KEY> !== null) ? this.p<RESEND_API_KEY>(x, {
                    section_index,
                }) : [x];
...
export class BloomTokenizer extends GPT2Tokenizer { // NOTE: `GPT2Tokenizer` to get the correct chat template
    ...
    constructor(tokenizerJSON, tokenizerConfig) {
        ...
        const splitChars = '.,!?\u2026\u3002\uff0c\u3001\u0964\u06d4\u060c';
        const patternObject = tokenizerJSON.p<RESEND_API_KEY>?.pretokenizers[0]?.pattern;
...
export class LlamaTokenizer extends PreTrainedTokenizer {
    ...
    constructor(tokenizerJSON, tokenizerConfig) {
        ...
        if (!this.legacy) {
            ...
            this.p<RESEND_API_KEY> = new MetaspacePreTokenizer({
