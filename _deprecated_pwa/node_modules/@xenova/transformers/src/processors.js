 ...
 * @param {string} featu<RESEND_API_KEY> The name of the feature extractor.
 * @private
 */
function validate_audio_inputs(audio, featu<RESEND_API_KEY>) {
    if (!(audio instanceof Float32Array || audio instanceof Float64Array)) {
        throw new Error(
            `${featu<RESEND_API_KEY>} expects input to be a Float32Array or a Float64Array, but got ${audio?.constructor?.name ?? typeof audio} instead. ` +
...
export class ImageFeatureExtractor extends FeatureExtractor {
    ...
    get_resize_output_image_size(image, size) {
        ...
        if (shortest_edge !== undefined || longest_edge !== undefined) {
            ...
            if (this.config.keep_aspect_ratio && this.config.ensu<RESEND_API_KEY>) {
                ...
                newHeight = constraint_to_multiple_of(scale_height * srcHeight, this.config.ensu<RESEND_API_KEY>);
                newWidth = constraint_to_multiple_of(scale_width * srcWidth, this.config.ensu<RESEND_API_KEY>);
            }
...
export class Processor extends Callable {
     ...
     * @param {FeatureExtractor} featu<RESEND_API_KEY> The function used to extract features from the input.
     */
    constructor(featu<RESEND_API_KEY>) {
        super();
        this.featu<RESEND_API_KEY> = featu<RESEND_API_KEY>;
        // TODO use tokenizer here?
    }

    /**
     * Calls the featu<RESEND_API_KEY> function with the given input.
     * @param {any} input The input to extract features from.
     * @param {...any} args Additional arguments.
     * @returns {Promise<any>} A Promise that resolves with the extracted features.
     */
    async _call(input, ...args) {
        return await this.featu<RESEND_API_KEY>(input, ...args);
    }
}
...
export class SamProcessor extends Processor {
     ...
     */
    async _call(...args) {
        return await this.featu<RESEND_API_KEY>(...args);
    }
    ...
    post_process_masks(...args) {
        // @ts-ignore
        return this.featu<RESEND_API_KEY>(...args);
    }
    /**
...
export class WhisperProcessor extends Processor {
    /**
     * Calls the featu<RESEND_API_KEY> function with the given audio input.
     * @param {any} audio The audio input to extract features from.
     * @returns {Promise<any>} A Promise that resolves with the extracted features.
     */
    async _call(audio) {
        return await this.featu<RESEND_API_KEY>(audio)
    }
}
...
export class SpeechT5Processor extends Processor {
     ...
     */
    async _call(input) {
        return await this.featu<RESEND_API_KEY>(input)
    }
}
...
export class AutoProcessor {
     ...
     * 
     * The processor class to instantiate is selected based on the `featu<RESEND_API_KEY>` property of the config object
    ...
    static async from_pretrained(pretrained_model_name_or_path, {
        ...
        let key = preprocessorConfig.featu<RESEND_API_KEY> ?? preprocessorConfig.image_processor_type;
        let featu<RESEND_API_KEY> = this.FEATURE_EXTRACTOR_CLASS_MAPPING[key];

        if (!featu<RESEND_API_KEY>) {
            if (preprocessorConfig.size !== undefined) {
                ...
                featu<RESEND_API_KEY> = ImageFeatureExtractor;
            } else {
        ...
        let featu<RESEND_API_KEY> = new featu<RESEND_API_KEY>(preprocessorConfig);
        return new processor_class(featu<RESEND_API_KEY>);
    }
}
//////////////////////////////////////////////////
