 ...
 * ```javascript
 * const image_featu<RESEND_API_KEY> = await pipeline('image-feature-extraction', 'Xenova/vit-base-patch16-224-in21k');
 * const url = 'https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/cats.png';
 * const features = await image_featu<RESEND_API_KEY>(url);
 * // Tensor {
 * //   dims: [ 1, 197, 768 ],
 * //   type: 'float32',
 ...
 * ```javascript
 * const image_featu<RESEND_API_KEY> = await pipeline('image-feature-extraction', 'Xenova/clip-vit-base-patch32');
 * const url = 'https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/cats.png';
 * const features = await image_featu<RESEND_API_KEY>(url);
 * // Tensor {
 * //   dims: [ 1, 512 ],
...
export class AudioClassificationPipeline extends (/** @type {new (options: AudioPipelineConstructorArgs) => AudioClassificationPipelineType} */ (Pipeline)) {
    ...
    async _call(audio, {
        ...
        const sampling_rate = this.processor.featu<RESEND_API_KEY>;
...
export class AutomaticSpeechRecognitionPipeline extends (/** @type {new (options: TextAudioPipelineConstructorArgs) => AutomaticSpeechRecognitionPipelineType} */ (Pipeline)) {
    ...
    async _call_whisper(audio, kwargs = {}) {
        ...
        const time_precision = this.processor.featu<RESEND_API_KEY> / this.model.config.max_source_positions;
        const hop_length = this.processor.featu<RESEND_API_KEY>;
    ...
    async _call(images, {
        ...
        if (subtask !== null) {
            ...
            for (let [task, func] of Object.entries(this.subtasks_mapping)) {
                if (func in this.processor.featu<RESEND_API_KEY>) {
                    fn = this.processor.featu<RESEND_API_KEY>[func].bind(this.processor.featu<RESEND_API_KEY>);
                    subtask = task;
                    break;
                }
            }
...
export class ObjectDetectionPipeline extends (/** @type {new (options: ImagePipelineConstructorArgs) => ObjectDetectionPipelineType} */ (Pipeline)) {
    ...
    async _call(images, {
        ...
        // @ts-ignore
        const processed = this.processor.featu<RESEND_API_KEY>(output, threshold, imageSizes);
...
export class ZeroShotObjectDetectionPipeline extends (/** @type {new (options: TextImagePipelineConstructorArgs) => ZeroShotObjectDetectionPipelineType} */ (Pipeline)) {
    ...
    async _call(images, candidate_labels, {
        ...
        for (let i = 0; i < preparedImages.length; ++i) {
            ...
            // @ts-ignore
            const processed = this.processor.featu<RESEND_API_KEY>(output, threshold, imageSize, true)[0];
