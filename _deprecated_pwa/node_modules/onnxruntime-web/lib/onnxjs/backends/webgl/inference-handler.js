...
const textu<RESEND_API_KEY> = require("./texture-layout");
...
class WebGLInferenceHandler {
    ...
    calculateTextureWidthAndHeight(shape, textureType) {
        return (0, textu<RESEND_API_KEY>)(this.session.layoutStrategy, shape, textureType);
    }
    executeProgram(program, inputs) {
        ...
        const outputTextureLayout = (0, textu<RESEND_API_KEY>)(this.session.layoutStrategy, programInfo.output.dims, programInfo.output.textureType);
    ...
    getOrCreateTextureData(tensor, textureType) {
        ...
        if (!td) {
            const layout = (0, textu<RESEND_API_KEY>)(this.session.layoutStrategy, tensor.dims, textureType);
            if (textureType === types_1.TextureType.packedLastDimension) {
                ...
                if (shape.length === 4) {
                    ...
                    const adjustedLayout = (0, textu<RESEND_API_KEY>)(this.session.layoutStrategy, adjustedKernelShape, textureType);
            ...
            if (textureType === types_1.TextureType.packed) {
                const unpackedTextureLayout = (0, textu<RESEND_API_KEY>)(this.session.layoutStrategy, tensor.dims, 1, [], { reverseWH: true });
