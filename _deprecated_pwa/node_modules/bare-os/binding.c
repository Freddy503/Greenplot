...
static uv_rwlock_t ba<RESEND_API_KEY>;

static uv_once_t ba<RESEND_API_KEY> = UV_ONCE_INIT;

static void
ba<RESEND_API_KEY>(void) {
  int err = uv_rwlock_init(&ba<RESEND_API_KEY>);
  assert(err == 0);
}
...
static js_value_t *
ba<RESEND_API_KEY>(js_env_t *env, js_callback_info_t *info) {
  int err;
...
static int
ba<RESEND_API_KEY>(uint32_t num) {
  int result = 0;
...
ba<RESEND_API_KEY>(js_env_t *env, js_callback_info_t *info) {
  ...
  for (size_t i = 0, n = len; i < n; i++) {
    ...
    if (family == AF_INET) {
      ...
      cidr_value = ba<RESEND_API_KEY>(address.netmask.netmask4.sin_addr.s_addr);
    } else {
      ...
      for (size_t i = 0; i < 16; i++) {
        uint32_t count = ba<RESEND_API_KEY>(address.netmask.netmask6.sin6_addr.s6_addr[i]);
...
ba<RESEND_API_KEY>(js_env_t *env, js_callback_info_t *info) {
  ...
  uv_rwlock_rdlock(&ba<RESEND_API_KEY>);

  err = uv_os_environ(&items, &len);

  uv_rwlock_rdunlock(&ba<RESEND_API_KEY>);
...
ba<RESEND_API_KEY>(js_env_t *env, js_callback_info_t *info) {
  ...
  uv_rwlock_wrlock(&ba<RESEND_API_KEY>);

  err = uv_os_setenv((char *) name, (char *) value);

  uv_rwlock_wrunlock(&ba<RESEND_API_KEY>);
...
static js_value_t *
ba<RESEND_API_KEY>(js_env_t *env, js_value_t *exports) {
  uv_once(&ba<RESEND_API_KEY>, ba<RESEND_API_KEY>);
...
#define V(name, fn) \
  ...
  V("type", bare_os_type)
  V("version", ba<RESEND_API_KEY>)
  V("release", ba<RESEND_API_KEY>)
  V("machine", ba<RESEND_API_KEY>)
  V("execPath", ba<RESEND_API_KEY>)
  V("pid", bare_os_pid)
  V("ppid", bare_os_ppid)
  V("cwd", bare_os_cwd)
  V("chdir", ba<RESEND_API_KEY>)
  V("tmpdir", ba<RESEND_API_KEY>)
  V("homedir", ba<RESEND_API_KEY>)
  V("hostname", ba<RESEND_API_KEY>)
  V("kill", bare_os_kill)
  V("availableParallelism", ba<RESEND_API_KEY>)
  V("cpuUsage", ba<RESEND_API_KEY>)
  V("threadCpuUsage", ba<RESEND_API_KEY>)
  V("resourceUsage", ba<RESEND_API_KEY>)
  V("memoryUsage", ba<RESEND_API_KEY>)
  V("freemem", ba<RESEND_API_KEY>)
  V("totalmem", ba<RESEND_API_KEY>)
  V("availableMemory", ba<RESEND_API_KEY>)
  V("constrainedMemory", ba<RESEND_API_KEY>)
  V("uptime", ba<RESEND_API_KEY>)
  V("loadavg", ba<RESEND_API_KEY>)
  V("cpus", bare_os_cpus)
  V("getProcessTitle", ba<RESEND_API_KEY>)
  V("setProcessTitle", ba<RESEND_API_KEY>)
  V("getPriority", ba<RESEND_API_KEY>)
  V("setPriority", ba<RESEND_API_KEY>)
  V("getEnvKeys", ba<RESEND_API_KEY>)
  V("getEnv", ba<RESEND_API_KEY>)
  V("hasEnv", ba<RESEND_API_KEY>)
  V("setEnv", ba<RESEND_API_KEY>)
  V("unsetEnv", ba<RESEND_API_KEY>)
  V("userInfo", ba<RESEND_API_KEY>)
  V("networkInterfaces", ba<RESEND_API_KEY>)
#undef V
...
BARE_MODULE(bare_os, ba<RESEND_API_KEY>)
