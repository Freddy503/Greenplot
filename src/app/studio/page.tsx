        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 18px', height: 56 }}>
          ...
          {editing ? (
            ...
            <>
              ...
              {prd.autoGenerated && prd.sourcePaperId && (
                <button
                  onClick={async () => {
                    const token = localStorage.getItem('greenplot_token')
                    const res = await fetch(`/api/papers/${prd.sourcePaperId}/draft-prd?replace=${encodeURIComponent(prd.id)}`, {
                      method: 'POST',
                      headers: token ? { Authorization: `Bearer ${token}` } : {},
                    }).catch(() => null)
                    if (res?.ok) toast.success('Regenerating with the v2 rubric — reopen this PRD in ~2 min', { duration: 7000 })
                    else toast.error('Could not start regeneration — backend update pending')
                  }}
                  className="tap"
                  title="Regenerate this draft with the engineering-grade rubric"
