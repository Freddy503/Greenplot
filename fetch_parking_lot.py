...
try:
    with urllib.request.urlopen(req) as r:
        ...
        print(f"Found {len(res.get('results', []))} rated seeds:")
        for p in res['results']:
            title = ''.join(x['plain_text'] for x in p['properties']['Thought']['title'])
            rating_prop = p['properties'].get('Seed Rating', {})
            rating = rating_prop.get('select', {}) if rating_prop else {}
            feedback_prop = p['properties'].get('Feedback', {})
            feedback = ''.join(x['plain_text'] for x in feedback_prop.get('rich_text', [])) if feedback_prop else ''
