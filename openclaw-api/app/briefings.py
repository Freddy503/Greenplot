...
Uses:
- OpenRouter (via OpenAI SDK) for LLM: Nemotron Super (nvidia/llama-3.3-nemotron-super-49b-v1:free)
...
def _call_llm(prompt: str, system: str = "", max_tokens: int = 1500, model: str = None) -> str:
    ...
    Default model: Nemotron Super (nvidia/llama-3.3-nemotron-super-49b-v1:free)
    ...
    try:
        ...
        # Try fallback model
        if model != "nvidia/llama-3.3-nemotron-super-49b-v1:free":
            logger.info("Retrying with Nemotron fallback...")
            return _call_llm(prompt, system, max_tokens, model="nvidia/llama-3.3-nemotron-super-49b-v1:free")
        return ""
...
"""
    ...
    deep_pattern = _call_llm(
        llm_prompt,
        max_tokens=500,
        model="nvidia/llama-3.3-nemotron-super-49b-v1:free"  # Nemotron is more reliable
    )
    if not deep_pattern:
...
"""
    ...
    news_synthesis = _call_llm(
        news_prompt,
        max_tokens=600,
        model="nvidia/llama-3.3-nemotron-super-49b-v1:free"  # Longer context for news synthesis
    )
...
"""
    ...
    evaluation = _call_llm(
        eval_prompt,
        max_tokens=400,
        model="nvidia/llama-3.3-nemotron-super-49b-v1:free"  # Longer context for analysis
    )
    if not evaluation:
...
"""
    ...
    challenge = _call_llm(
        challenge_prompt,
        max_tokens=500,
        model="nvidia/llama-3.3-nemotron-super-49b-v1:free"  # Creative synthesis task
    )
    if not challenge:
